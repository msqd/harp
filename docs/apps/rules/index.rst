...
.. warning::
    ...
    Early prototype. The API may change, a lot. We chosed to expose a rather low level API, because we think
    it's easier to go up than down, and we have no idea what use cases our users may come with. If your use case is not
    covered by the current API, or hard to implement, please open a discussion thread about it.
